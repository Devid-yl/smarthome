...
class RealtimeHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        print(f"[WebSocket] Client connecté (user_id={user_id}). Total clients: {len(RealtimeHandler.clients)}")
    ...
    def on_close(self):
        ...
        print(f"[WebSocket] Client déconnecté. Total clients: {len(RealtimeHandler.clients)}")
    
    @classmethod
    def broadcast_sensor_update(
        cls, sensor_id: int, value: float,
        is_active: bool, house_id: int | None = None
    ):
        """
        ...
        print(f"[WebSocket] Broadcasting sensor update: sensor_id={sensor_id}, value={value}")
        dead_clients = set()
    ...
    def broadcast_equipment_update(
        cls, equipment_id: int, equipment_type: str,
        state: str, is_active: bool, house_id: int | None = None
    ):
        """
        ...
        print(f"[WebSocket] Broadcasting equipment update: equipment_id={equipment_id}, state={state}")
        ...
        for client in cls.clients:
            try:
                client.write_message(message)
                sent_count += 1
                print(f"[WebSocket] Message envoyé au client (user_id={client.user_id})")
            except Exception as e:
                print(f"[WebSocket] Error sending to client: {e}")
                dead_clients.add(client)
        
        print(f"[WebSocket] Broadcast terminé: {sent_count}/{len(cls.clients)} messages envoyés")
